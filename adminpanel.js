...
//   })
  ...
  function pagination(collectionName, pagePath, res, req, data) {
    ...
    db.collection(collectionName).find({}).sort({ _id: -1 }).skip(skip).limit(perPage).toArray((err, alldata) => {
      db.collection(collectionName).countDocuments((err, userCount) => {
        ...
        var moneyCount = 0
        if (typeof alldata[0].walletBalance != 'undefined') {
          for (var i = 0; i < alldata.length; i++) {
            moneyCount += parseInt(alldata[i].walletBalance)
          }
        }
        data['allBalance'] = moneyCount;
  ...
  function responseData(file, data, res) {
    ...
    // data['active'] = typeof sess.active != 'undefined' ? sess.active : 'login';
  ...
  //   db.collection('register').aggregate(aggregate).toArray((err, myres) => {
  //     res.send(myres)
  //   })
  // })
    ...
    if (sess.Mobile_no != undefined || sess.Mobile_no != null) {
      ...
      db.collection('todaypin').find({}).sort({ _id: 1 }).limit(1).toArray((err, mypin) => {
        ...
        for (var i = 0; i < kk.length; i++) {
          db.collection('pinHistory').find({ _id: objectId(kk[i]), use: 0 }).sort({ _id: 1 }).toArray((err, mylastpin) => {
            console.log(mylastpin, 'xxx')
 ...
 /* app.get('/ph_rh/:page', (req, res) => {
  ...
  app.get('/phrh/:page', (req, res) => {
    ...
    // if (sess.Mobile_no != undefined || sess.Mobile_no != null) {
      ...
      db.collection('todaypin').find({}).sort({_id: 1}).limit(1).toArray((err, firstpin) => {
        ...
        const date2 = new Date(firstpin[0].date);
        date2.setDate(date2.getDate() + 3)
        date2.setUTCHours(23, 59, 59, 59);
  ...
  app.post('/giveph_rh/:_id', (req, res) => {
    sess = req.session;
    if (sess.Mobile_no != undefined || sess.Mobile_no != null) {
      ...
      db.collection('todaypin').find({visible: 0}).sort({ _id: 0 }).limit(1).sort({ _id: -1 }).toArray((err, mypin) => {
        ...
        db.collection('pinHistory').findOne({ _id: objectId(req.params._id) }, (err, myoid) => {
          console.log(myoid, 'aa')
          db.collection('pinHistory').findOne({_id: { $nin: [objectId(myoid.User_id)] }, makeReceiver: 1 }, (err, oneitem) => {
  ...
  app.get('/rh_link/:page', (req, res) => {
    ...
    { $project: {
      ...
      date: '$date'
          ...
          db.collection('adminAddAmount').aggregate(aggregate).sort({ _id: -1 }).skip(skip).limit(perPage).toArray((err, alldata) => {
            var mytotal = [{
              $lookup: {
                from: "register",
                localField: "User_id",
          ...
          {$count: 'total'}]
            db.collection('adminAddAmount').aggregate(mytotal).toArray((err, userCount) => {
    ...
    app.post('/amountaddhistory/:page', (req, res) => {
      ...
      {$match: wh},
          ...
          db.collection('adminAddAmount').countDocuments(wh, (err, userCount) => {
              db.collection('adminAddAmount').aggregate(aggregate).skip(skip).limit(perPage).toArray((err, users) => {
      ...
      db.collection('register').find({}).sort({ _id: -1 }).skip(skip).limit(perPage).toArray((err, alldata) => {
        db.collection('register').countDocuments((err, userCount) => {
          ...
          db.collection('register').aggregate(aggregate).toArray((err, allBalances) => {
            ...
            if(allBalances[0] == undefined || allBalances[0]==null){
              allBalances[0] = 0
            }
   ...
   app.post('/updatewalletbyadmin', (req, res) => {
     ...
     if(sess.Mobile_no != undefined || sess.Mobile_no != null){
       ...
       if('+' == data.price){
         ...
         var var query = {User_id: kk[i]}
      var value = {$set: {use: 1}}
      db.collection('pinHistory').updateOne(query, value) = { _id: objectId(data._id)};
        ...
        db.collection('phrh').countDocuments({ sender_id: objectId(req.params._id) }, (err2, phcount) => {
          db.collection('phrh').countDocuments({ receiver_id: objectId(req.params._id) }, (err3, rhcount) => {
            db.collection('pinHistory').find({ User_id: objectId(req.params._id) }).sort({ _id: -1 }).toArray((err4, phistory) => {
              db.collection('walletTransfer').find({ User_id: objectId(req.params._id) }).sort({ _id: -1 }).toArray((err4, wt) => {
